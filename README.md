...
SLO regression (성능 목표 후퇴) 을 nightly CI (매일 야간 자동 빌드) 로 검증합니다.
p95 > 300ms 또는 5xx > 1% 면 fail.
...
Pod 가 만들어지기 직전에 정책을 검사해서 거절할 수 있는 훅) 가 admission 단계에서:
...
- Resilience4j 서킷 브레이커 — 외부 호출이 자꾸 실패하면 일정 시간 동안 호출 자체를
  막아 장애가 우리 쪽으로 번지지 않게 격리 (K8s API / Kafka 장애 격리)
- Job timeout 감시 작업 (Spring Scheduling + ShedLock — 여러 인스턴스 중 하나만
  스케줄을 돌리도록 잠금)
- Outbox 다중 인스턴스 처리 (`SKIP LOCKED` — 다른 트랜잭션이 잠근 row 는 건너뛰고
  남은 것만 가져가는 PostgreSQL 기능, 또는 ShedLock)
