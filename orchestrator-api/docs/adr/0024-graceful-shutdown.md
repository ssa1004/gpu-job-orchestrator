...
K8s 가 Pod 종료 시 SIGTERM 을 보낸다. default 30초 뒤에도 살아 있으면 SIGKILL 이다.
Spring Boot 의 default 동작은 SIGTERM 을 받자마자 web container 를 멈추고 응답 중인
요청을 끊고 종료한다. 사용자 입장에서는 영문 모를 5xx 가 가끔씩 발생하는 모양이 된다.
배포 / 스케일 다운 / node drain 마다 일정 비율의 요청이 깨진다.

배포가 하루에 5번이고 평균 RPS 가 100 이라면, 매 배포마다 500ms x 100 = 50개의
in-flight 요청이 강제 종료된다. 사용자는 왜 가끔 5xx 가 나오는지 추적할 수 없고,
운영에서 늘 따라다니는 미세한 잡음으로 남는다.

표준 해법은 graceful shutdown 이다. SIGTERM 후 다음 순서로 진행한다.
...
이 흐름이 30초 안에 끝나면 사용자 입장에서 5xx 가 0건이 된다.
...
T=0~5s preStop hook: sleep 5
       ...
       이 동안 들어오는 새 요청은 아직 받는다 (Spring 이 거절하지 않음).
...
으로 설정한다. 그 결과 다른 어떤 cleanup 보다 먼저 readiness 가 빠지는 것이 보장된다.

readiness 가 늦게 빠지면 그 사이 들어오는 새 요청이 in-flight 풀에 합류해서 처리할
요청 수가 계속 늘어난다. timeout-per-shutdown-phase 안에 끝내지 못하게 된다.

### 4) 왜 SmartLifecycle 이고 @PreDestroy 가 아닌가

@PreDestroy 는 ApplicationContext close 의 마지막 단계라서 이미 web container 가 멈춘
후에 호출된다. 그때 readiness 를 publish 해도 의미가 없다 (web container 가 응답하지
못한다).

SmartLifecycle.stop() 은 ApplicationContext close 의 시작 단계라 web container 가
살아 있는 동안 publish 가 가능하고, 다음 readinessProbe 응답에 즉시 반영된다.

### 5) 왜 K8s preStop hook 만으로는 부족한가

preStop hook 은 컨테이너에 SIGTERM 을 보내기 전에 실행되는 K8s 단계다. `sleep 5` 만
넣으면 그 동안 endpoint 갱신 propagation 은 진행되지만, Spring 자체는 여전히 ready 라고
답한다. 어떤 LB 는 health-check 결과를 endpoint state 보다 우선시한다. 결과적으로 이
Pod 가 endpoint 에서 안 빠질 수 있다.

SmartLifecycle 로 readiness 를 Spring 안에서 명시적으로 빼야 모든 LB 에 대해 일관된
신호를 보낼 수 있다.
...
K8s 위에서 운영하는 서비스에 graceful shutdown 이 없으면 매 배포마다 일정 확률로 5xx 가
나온다. 시나리오는 명확하다.
...
- **graceful shutdown (우아한 종료)** — Pod를 끌 때 처리 중인 요청을 끊어버리지 않고, 새 요청만 막은 뒤 진행 중인 것을 마저 끝내고 정리한 다음 꺼지는 것. 영업 종료 시 새 손님은 안 받되 안에 있는 손님 계산은 끝내주는 셈.
- **SIGTERM / SIGKILL** — SIGTERM은 "이제 정리하고 그만 꺼"라는 정중한 종료 신호, SIGKILL은 그래도 안 꺼지면 보내는 강제 전원 차단. 그 사이(기본 30초)에 깔끔히 정리해야 한다.
- **in-flight 요청** — 종료 신호가 온 순간 아직 처리 중이던 요청들. 이걸 안 기다리고 끄면 사용자에게 영문 모를 5xx 에러가 뜬다.
- **preStop hook** — 컨테이너에 종료 신호를 보내기 직전 K8s가 잠깐 실행하는 단계. 여기서 잠시 기다려 "이 Pod는 빼라"는 정보가 로드밸런서까지 퍼질 시간을 번다.
- **readiness 빼기** — 종료 시작과 동시에 "나 이제 트래픽 받지 마"를 가장 먼저 알려 새 요청 유입을 끊는 것. 이게 늦으면 처리할 요청이 계속 늘어 제때 못 끝낸다.
