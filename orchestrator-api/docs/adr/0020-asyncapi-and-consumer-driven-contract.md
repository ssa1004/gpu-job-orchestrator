...
- **AsyncAPI** — REST(요청-응답)용 명세인 OpenAPI의 이벤트 버전. "어떤 채널에 어떤 메시지가 흐르는지"를 적어 발행자와 구독자가 같은 약속을 보게 한다.
- **consumer-driven contract (소비자 주도 계약, CDC)** — 메시지를 받는 쪽이 "나는 이 필드들을 쓴다"를 명시로 박아두고, 보내는 쪽이 그걸 깨면 빌드가 실패하게 하는 것. 받는 사람 입장을 미리 못 박아 깜빡 변경 사고를 막는다.
- **schema evolution / backward-compat (스키마 진화·하위호환)** — 메시지 형식이 바뀌어도 기존 소비자가 안 깨지게 조심하는 것. 필드 추가는 안전(OK)이지만 필수 필드 제거·이름 변경은 깨짐(BREAKING).
- **sealed interface (봉인된 인터페이스)** — 구현 종류를 정해진 목록으로만 제한한 타입. 발행되는 이벤트 종류가 이 목록에 고정돼, 새 종류를 빠뜨리면 검증에서 바로 걸린다.
- **monorepo (모노레포)** — 여러 서비스 코드를 한 저장소에 같이 두는 방식. 그래서 별도 브로커 인프라 없이 저장소 안 파일만으로 계약 검증이 가능하다.
