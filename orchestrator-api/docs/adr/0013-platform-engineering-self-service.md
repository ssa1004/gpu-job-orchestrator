...
- **self-service (셀프 서비스)** — 개발팀이 "namespace 만들어 주세요" 하고 플랫폼팀에 일일이 요청하지 않고, 포털에서 직접 신청해 받아가는 것. 은행 창구 대기 대신 ATM에서 알아서 처리하는 셈.
- **IDP (Internal Developer Platform, 사내 개발자 포털)** — 인프라 신청·생성을 개발자가 직접 할 수 있게 만든 사내 전용 셀프 서비스 창구.
- **IRSA (IAM Roles for Service Accounts)** — Pod가 AWS 자원(S3 등)에 접근할 때 쓰는 권한을, 사람이 키를 박아두지 않고 K8s 계정과 자동으로 묶어 발급하는 방식.
- **abstraction leak (추상화 누수)** — 복잡한 내부를 간단한 양식으로 가려놨지만, 특이한 요구가 들어오면 가려둔 복잡함이 다시 새어 나오는 현상. 결국 그런 건 다시 사람 손으로 처리해야 한다.
