...
지금까지 `ShedLock` (DB row 락 기반 라이브러리, ADR 없는 채로 도입됨) 으로 처리. row 한 개를
`SELECT ... FOR UPDATE` 로 잠그고 메서드 종료 시 lock_until 갱신 — 동작은 한다. 그러나 K8s
위에 배포된 시스템 전체를 보면 이 메커니즘은 *환경에 비해 어색* :

- DB 가 down 이면 leader election 자체가 마비 (스케줄러 정지)
- Pod 죽음을 DB 가 *모른다* — `lock_until` 만료까지 다른 인스턴스가 takeover 못 함 (보통 1~5분)
- ShedLock 테이블 자체가 sysadmin 으로부터 *왜 있는 건지* 질문 받음 — 도메인 무관

K8s 자체가 이미 *합의된 분산 락 서비스* 를 제공한다. controller-runtime,
kube-controller-manager, kube-scheduler 가 모두 이 메커니즘으로 leader 를 선출:
`coordination.k8s.io/Lease` 리소스. etcd 가 backend 라 강한 일관성 보장 (Raft consensus).
...
표준 시간값 (kube-controller-manager 와 동일):
...
비율 (15 / 10 / 2) 의 트레이드오프: 너무 짧으면 (5s) network blip 한 번에 leader 가 바뀌어 churn
(잦은 전환), 너무 길면 (60s) takeover 가 느려 SLA 영향. 15s 가 검증된 합리값.
...
한 줄로 확인 가능. ShedLock 의 `locked_by` 와 비슷하지만 K8s native 라 도구 (k9s / Lens) 가 자동 인식.
...
2. **lease-duration 갭** — 리더가 죽고 lease 만료 직전 (예: 14s 시점) 에 새 리더가
   나타나면 *둘 다 자기가 리더라고 믿는* split-brain 의 짧은 윈도우. 이 윈도우 안에 두 인스턴스의
   tick 이 우연히 겹치면 ShedLock 가 두 번째를 거절 → 안전.
...
핵심: *불확실하면 false* . 두 인스턴스가 동시에 자기가 리더라고 믿는 split-brain 이 outbox
중복 발행 / preemption race 등 데이터 사고로 이어질 수 있어 보수적으로 false 유지.
...
namespace=gwp 로 제한 — ClusterRole 이 아닌 namespace Role 로 권한 폭증 방지. fabric8 가
필요로 하는 verb 셋 (kube-controller-manager 와 동일).
...
- `kubectl get lease` 로 *지금 누가 리더인지* 한 줄로 확인 (운영 가시성 ↑)
...
에서 *현재 어떤 Pod 가 리더인지* 시각화 가능. (후속 작업)
