...
- 누락 사고 0 — 같은 트랜잭션 + UNIQUE 멱등.
- 단가 박제로 *과거 cost 가 변하지 않음*.
- 4가지 view 가 모두 같은 ledger 를 출처로 일관성.
- `Job` aggregate 가 회계 책임에서 자유 — 도메인 cleaner.

### 비용 / 한계

- 종착 path 가 늘어날 때마다 hook 추가 필요 (현재 5곳). 누락 검출은 통합 테스트로.
- 단일 단가만 지원 — instance type 별 (A100/H100/V100) 다른 rate 는 후속 ADR 의 `GpuRateCard`.
- `recordedAt` 기준 query — 잡이 4월에 시작해서 5월에 끝나면 5월 청구. 정책 다르면 별도 인덱스 추가.
