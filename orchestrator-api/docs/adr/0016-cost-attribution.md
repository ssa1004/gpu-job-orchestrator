...
GPU 클러스터 운영에서 누가 얼마나 썼고 그래서 얼마인지가 매달 따라온다.
...
이전까지 잡의 종착 사실은 `Job` aggregate 에만 있고, 실제 cost 환산은 외부 spreadsheet
또는 사후 처리 batch (잡이 끝난 뒤 별도 일괄 처리) 로 했다. 그 결과 다음 문제가 누적됐다.

- 어느 시점 단가를 적용한 것인지 불명확 (단가가 바뀌면 과거 잡의 cost 가 흐트러진다)
- Job 이 SUCCEEDED 됐는데 cost 가 누락되는 회계 사고가 가능
- 빌링 / dashboard / 청구서가 각각 다른 쿼리로 같은 잡을 다르게 합산
...
job_cost_records
  ...
  ├─ owner, gpu_count
  ├─ runtime_millis
  ├─ rate_per_gpu_hour    ← 계산 시점 단가 스냅샷 (FeeSnapshot)
  ├─ computed_cost
  ├─ currency             ← 기본 KRW
  ├─ final_status         ← SUCCEEDED / FAILED / CANCELLED / PREEMPTED 그대로 보관
...
- `Job` 은 상태 변경의 aggregate, `JobCostRecord` 는 불변 사실의 ledger.
- 가격 정정 / 재계산이 필요할 때는 새 row 로 표현한다 (append-only). Job 컬럼을 update
  하면 history 가 사라진다.
- Cost analytics query 가 Job 의 다른 컬럼 (k8s 이름 / errorMessage / resultUri) 을 join
  하지 않아도 빨리 끝난다.

### 계산 시점 단가 스냅샷 (Snapshot 패턴 — 그 시점의 값을 그대로 보존하는 패턴)

`CostRate` (record VO — 값을 담는 불변 객체) 에는 `costPerGpuHour` 한 필드만 둔다.
`CostRateProvider` 가 `application.yml` 의 `gwp.cost.gpu-hour-rate-krw` 를 읽어 제공한다.

핵심은 잡 종착 시점의 `rate_per_gpu_hour` / `computed_cost` 를 row 에 그대로 보관한다는
점이다. 단가가 나중에 바뀌어도 과거 row 는 영향을 받지 않는다 (FeeSnapshot /
PricingSnapshot 패턴).

예: 5월 1일 단가가 5,000원 → 6월 1일에 6,000원으로 인상 → 7월 1일에 5월에 끝난 잡 X 의
청구액을 다시 봐도 5,000원 기준 그대로다. 계산을 다시 하지 않는다. 회계 / 빌링 일관성이
깨지지 않게 하기 위함이다.
...
같은 트랜잭션에 묶는 이유는 회계 사고를 막기 위해서다. Job SUCCEEDED 가 commit 됐는데
cost record 만 누락되면 청구가 비는 사고가 된다. Job 과 cost record 를 함께 commit /
함께 rollback 하므로 누락이 발생할 수 없다.
...
- `startedAt == null` (dispatch 실패로 RUNNING 안 함) → runtime 0, cost 0, **record 는
  만든다**. 어떤 잡이 dispatch 실패였는지 운영에서 추적할 수 있게 하기 위함이다.
...
- 누락 사고 가능성 (batch 가 한 번 실패하면 cost 가 영영 안 잡힌다)
- 그 시점 단가의 보존이 어려움 (사후 batch 가 도는 시점의 단가가 적용된다)
- Job SUCCEEDED ↔ cost record 사이 시간차로 billing 정확성에 흠이 생긴다
...
- 단가 스냅샷으로 과거 cost 가 단가 변경의 영향을 받지 않음
...
- instance type 별 `GpuRateCard` — DB 영속 + 시간대별 rate (현재는 owner 별 단일 rate provider)
- cost dashboard streaming — Outbox → Kafka → 실시간 집계 (현재는 ledger 누적 후 query)
- 다단계 currency — KRW / USD / 변환 시점 환율 스냅샷 (현재는 단일 통화 가정)

## 용어 풀이 (쉽게)

- **cost attribution / chargeback (비용 귀속·재청구)** — GPU를 누가 얼마나 써서 얼마가 나왔는지 따져 실제 쓴 팀에게 다시 청구하는 것. 공용 카드 명세를 각자 쓴 만큼 나눠 갚는 셈.
- **ledger (장부)** — 한 번 적으면 고치거나 지우지 않고 계속 줄만 쌓는 회계 기록. 정정이 필요하면 반대 줄을 새로 추가한다(은행 통장처럼).
- **append-only (추가 전용)** — 기존 줄은 절대 손대지 않고 새 줄만 덧붙이는 방식. 과거 기록이 사라지지 않아 회계 분쟁 때 증명이 된다.
- **단가 스냅샷 (price snapshot)** — 잡이 끝난 그 시점의 GPU 단가를 그대로 박아두는 것. 나중에 단가가 올라도 과거 청구액은 그때 기준 그대로 유지돼 회계가 흐트러지지 않는다.
- **aggregate (애그리거트)** — 함께 묶여서 한 덩어리로 다뤄지고 저장되는 도메인 단위. 여기선 잡 상태를 다루는 Job과 비용 기록만 다루는 JobCostRecord를 일부러 다른 덩어리로 나눴다.
