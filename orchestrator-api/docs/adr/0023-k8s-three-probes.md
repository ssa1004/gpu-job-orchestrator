...
K8s 위에 올린 Pod 의 가용성 신호는 *세 가지* — liveness, readiness, startup. 셋의
의미가 다른데 같은 endpoint 로 묶으면 운영 사고가 즉시 일어난다.

### 사고 시나리오 1: liveness 에 DB ping 을 넣는다

- DB 가 5초 끊기는 일시 장애 발생.
- 모든 Pod 의 livenessProbe 가 fail → K8s 가 *모든 Pod 를 동시에 재시작*.
- 재시작 후 모든 Pod 가 동시에 DB 첫 connection 을 잡으려 spike → 회복 중인 DB 에
  추가 부하 → 회복 더 늦어짐.
- 사용자 입장에서 5초 끊김이 5분 outage 로 증폭 (cold start cascade).
...
| **startup** | *시작 중* 인가 | Pod 재시작 (단, startup 성공할 때까지 다른 probe disabled) |
...
### 3) liveness 에는 *외부 의존성* 안 본다

기본 Spring `LivenessStateHealthIndicator` 는 ApplicationContext 가 살아있고
`LivenessState=CORRECT` 일 때만 200. DB / Kafka / Redis 같은 외부 의존성은
*일부러* 안 본다. liveness fail 의 정당한 원인:
...
### 4) readiness 는 *critical 외부 의존성* 만 본다

`ApplicationReadinessCoordinator` 가 Resilience4j 의 회로 상태를 readiness 신호
로 매핑:

- **Kafka circuit OPEN** → readiness=REFUSING_TRAFFIC. broker 가 영구 down 인
  상태로 트래픽 받아봐야 outbox 만 쌓이고 사용자에게는 무응답.
- **K8s API circuit OPEN** → readiness=REFUSING_TRAFFIC. dispatch 자체가 fast-fail
  로 떨어지는 상태에서 새 잡 제출 받지 않는 게 깔끔.

readiness 에 *안* 들어가는 의존성:

- **DB connection 일시 단절** — HikariCP 가 자체 retry 로 흡수. 잠시 끊긴 정도로
  unready 를 publish 하면 모든 Pod 가 *동시에* unready → 트래픽 안 들어옴 → 회복 →
  다시 ready 의 churn (잦은 전환).
- **Redis 일시 끊김** — Redis 는 cache. cache miss 로 떨어져도 path 살아있음.
...
Flyway 가 새 마이그레이션 적용 + Hibernate metadata + JIT warmup 까지 합쳐
보통 30~60초. 150초 여유로 넉넉. startup 이 끝나야 livenessProbe / readinessProbe 가
활성화되므로, *느린 시작* 으로 livenessProbe 가 조기 fail 시키지 않게 보호.

### 6) 의존성 회복 시 자동 ready 복귀

`ApplicationReadinessCoordinator` 의 `recomputeReadiness()` 가 회로의 모든 state
transition 마다 호출 — OPEN → HALF_OPEN → CLOSED 가 일어나면 자동으로 ready 복귀.
운영자가 수동으로 ready 를 publish 할 일 없음.

## 왜 over-spec 아닌가

이건 *기본 위생*. K8s 위에 올린 어떤 서비스든 probe 3종이 명시되어 있어야 한다.
안 하면 위 시나리오 1~3 중 하나는 *반드시* 일어난다 — 시간 문제일 뿐.

- liveness 가 DB ping 을 보면 → 시나리오 1 (DB 일시 장애 → 전체 재시작 cascade).
- probe 가 한 개면 → 시나리오 2 (느린 시작 → CrashLoopBackOff).
- probe 가 0 개면 → 시나리오 3 (JVM deadlock → 100% 5xx).

추가 코드 ~150 라인 + yml ~20 라인 + 단위 테스트 ~150 라인 — *예방하는 사고 1건* 으로
ROI 충분.
