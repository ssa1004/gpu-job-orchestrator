...
Spring Boot 3.3, Java 17, JPA + Flyway (DB 스키마 마이그레이션 도구), Spring Security
(OAuth2 + JWT), Redis, Kafka, fabric8-kubernetes-client (Java 용 Kubernetes API 클라이언트),
...
- [ ] `Idempotency-Key` 헤더 처리 — 같은 요청이 두 번 와도 한 번만 처리되게 막는 헤더
  (네트워크 재시도 안전성)
- [ ] Resilience4j 서킷 브레이커 — 외부 호출이 자주 실패하면 일정 시간 차단해서 장애가
  우리 쪽으로 번지지 않게 격리 (K8s API / Kafka 장애 격리)
- [ ] Job timeout 감시 작업 (Spring Scheduling + ShedLock — 여러 인스턴스 중 하나만
  스케줄을 실행하도록 잠금)
- [ ] PostgreSQL advisory lock (애플리케이션이 직접 잠금 키를 정해 거는 PG 전용 락) 으로
  쿼터 동시성 강화
