...
/**
 * 외부 의존성 (Kafka broker, K8s API server) 호출 보호용 circuit breaker 빈.
 *
 * <p>Resilience4j-spring-boot3 starter 가 application.yml 의
 * {@code resilience4j.circuitbreaker.instances.<name>.*} 설정으로
 * {@link CircuitBreakerRegistry} 를 자동 wire. 이 클래스는 그 registry 에서
 * 이름 별로 인스턴스를 꺼내 명시적 빈으로 노출 — 호출 측 (OutboxRelay /
 * KubernetesJobDispatcher) 가 생성자 주입으로 받는다.</p>
 *
 * <p>인스턴스:</p>
 * <ul>
 *   <li>{@code kafka} — Outbox relay 의 Kafka send 호출 보호.</li>
 *   <li>{@code k8s} — fabric8 client 의 K8s API 호출 보호.</li>
 * </ul>
 *
