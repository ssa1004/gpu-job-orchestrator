 ...
 *
 * <p>설계: ownership 검증과 도메인 호출을 한 곳으로 모음. Controller 는 인증 정보 (owner,
 * isAdmin) 만 전달하고, 권한 검사 + 위임은 여기서 수행. {@link JobQueryService} /
 * {@link JobLifecycleService} 자체는 권한을 모름 — 내부 호출 (예: 시스템 작업, 콜백) 은
 * 권한 검사를 우회할 수 있도록 분리.</p>
 *
 * <p>읽기는 {@link JobQueryService#get} 으로 위임 → 다른 빈을 통한 호출이라 Spring AOP
 * 프록시를 거치고 `@Cacheable` 이 정상 동작 (self-invocation 으로 캐시 우회되는 문제
 * 방지).</p>
 */
@Service
