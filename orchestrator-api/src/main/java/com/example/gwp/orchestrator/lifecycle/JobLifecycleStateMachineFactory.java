...
import static com.example.gwp.orchestrator.domain.JobStatus.*;
import static com.example.gwp.orchestrator.lifecycle.JobLifecycleEvent.*;
 ...
 *
 * <h3>가드</h3>
 * <p>{@link JobLifecycleEvent#PREEMPT} 는 {@link PreemptionPolicy#PREEMPTABLE} 인 잡만 허용 —
 * 가드가 NEVER 정책을 reject. 도메인의 {@link Job#markPreempted} 도 같은 방어선이지만,
 * 상태 머신 단계에서 reject 하면 *왜 거절했는지* (NEVER 정책) 가 audit log 에 명확히 박힘.</p>
 */
@Configuration
public class JobLifecycleStateMachineFactory {

    @Bean
    public JobLifecycleStateMachine jobLifecycleStateMachine() {
        return build();
    }

    /** 테스트 / 외부 도구 (다이어그램 export) 가 직접 호출. */
    public static JobLifecycleStateMachine build() {
        // SUBMIT / SUBMIT_WITH_DEPS 는 *entry transition* (source 없음). 이 머신은 source-keyed
        // 이라 entry 는 다루지 않는다 — entry 는 도메인 ({@code Job.submit}) 에 위임하고
        ...
        List<Transition> transitions = List.of(
                ...
                Transition.guarded(QUEUED, PREEMPT, PREEMPTED, JobLifecycleStateMachineFactory::isPreemptable),
                Transition.guarded(DISPATCHING, PREEMPT, PREEMPTED, JobLifecycleStateMachineFactory::isPreemptable),
                Transition.guarded(RUNNING, PREEMPT, PREEMPTED, JobLifecycleStateMachineFactory::isPreemptable),
                Transition.guarded(WAITING_DEPS, PREEMPT, PREEMPTED, JobLifecycleStateMachineFactory::isPreemptable)
        );
        return new JobLifecycleStateMachine(transitions);
    }

    /**
     * Preempt 가드. context 가 {@link Job} 이면 그 잡의 preemption policy 검사. context 가
     * null 이거나 다른 타입이면 보수적으로 false (도메인이 별도로 검증).
     */
    static boolean isPreemptable(JobStatus source, JobLifecycleEvent event, Object context) {
        if (context instanceof Job job) {
            return job.getPreemptionPolicy() == PreemptionPolicy.PREEMPTABLE;
        }
        return false;
    }
}
