...
/**
 * K8s `coordination.k8s.io/v1/Lease` 리소스를 사용하는 leader election.
 *
 * ### 왜 K8s Lease 인가
 *
 * 이 시스템은 이미 K8s 위에 배포되고 fabric8 client / RBAC / namespace 가 다 갖춰져
 * 있다. K8s 자체가 합의된 분산 락 서비스를 무료로 제공한다. etcd 가 backend 라 Raft
 * consensus 기반의 강한 일관성을 보장한다. K8s 컨트롤 플레인의 표준 컴포넌트들도 같은
 * 메커니즘으로 leader 를 선출한다.
 *
 * 기존 ShedLock (DB row 락) 대비 장점:
 * - **K8s 가 leader liveness 자동 감지** — Pod 가 죽으면 (OOM / node failure /
 *   graceful shutdown) lease 갱신이 멈추고 lease 만료 후 즉시 다른 Pod 가 takeover.
 *   ShedLock 은 lock_until 까지 기다려야 함 — DB 가 죽음 자체를 모른다.
 * - **watch 기반 즉시 전환** — fabric8 LeaderElector 가 lease 변화를 watch 로
 *   감시 → 만료 즉시 acquire 시도. polling 지연 없음.
 * - **외부 의존성 0** — 이미 있는 K8s API server / kubelet / etcd 만 사용.
 *   DB 가 down 이어도 leader election 은 살아있다.
 * - **운영 가시성** — `kubectl get lease -n gwp gwp-orchestrator-leader`
 *   으로 *지금 누가 리더인지* 한 줄로 확인.
 *
 * ### 표준 시간값 (k8s 컨벤션)
 * ```
 ...
 * ```
 *
 * 이 비율 (15 / 10 / 2) 은 client-go 권장값으로 검증된 수치. lease-duration 이 너무
 * 짧으면 (5s) network blip 한 번에 leader 가 바뀌어 churn (잦은 전환), 너무 길면 (60s)
 * takeover 가 느려 SLA 영향. 15s 가 합리적 트레이드오프.
 *
 * ### 스레드 모델
 *
 * fabric8 LeaderElector 는 별도 단일 스레드에서 lease 갱신 / 감시. 메인 비즈니스 스레드는
 * [isLeader] 로 volatile flag 만 읽음 — 매 호출 nanosecond 단위. callback
 * (onStartLeading / onStopLeading) 이 와야만 flag 가 true / false 로 전환된다.
 *
 * ### fail-safe
 *
 * K8s API server 가 일시적으로 끊기면 lease 갱신 실패 → onStopLeading 호출 →
 * `leader=false` 로 전환. 그동안 스케줄러는 비-리더로 동작 (즉, no-op). API server
 ...
 * 을 방지.
 */
class KubernetesLeaseLeaderElector(
    private val client: KubernetesClient,
    private val namespace: String,
    private val leaseName: String,
    private val identity: String,
    private val leaseDuration: Duration,
    private val renewDeadline: Duration,
    private val retryPeriod: Duration,
) : LeaderElector {

    /** 리더 여부 — leader callback 스레드에서 set, 비즈니스 스레드에서 read. */
    private val leader = AtomicBoolean(false)

    /** fabric8 LeaderElector 의 long-running task. [stop] 에서 release 신호로 cancel. */
    private var running: CompletableFuture<*>? = null
    ...
    fun start() {
        ...
        this.executor = Executors.newSingleThreadScheduledExecutor { r ->
            Thread(r, "k8s-leader-elector-$leaseName").apply { isDaemon = true }
        }
        ...
        val callbacks = LeaderCallbacks(
            {
                leader.set(true)
                log.info(
                    "leader-election: this instance ({}) became LEADER for lease={}",
                    identity, leaseName,
                )
            },
            {
                leader.set(false)
                log.warn(
                    "leader-election: this instance ({}) STOPPED leading lease={}",
                    identity, leaseName,
                )
            },
            { newLeader ->
                log.debug(
                    "leader-election: lease={} new leader = {}",
                    leaseName, newLeader,
                )
            },
        )
        ...
        val elector = client.leaderElector().withConfig(config).build()
        ...
        log.info(
            "leader-election: started for lease={} ns={} identity={} duration={} renew={} retry={}",
    ...
    companion object {
        private val log = LoggerFactory.getLogger(KubernetesLeaseLeaderElector::class.java)
    }
}
