 ...
 *
 * 도메인 메서드의 [com.example.gwp.orchestrator.domain.IllegalJobTransitionException]
 * 과 별개 — 이건 *workflow 어휘* 의 위반, 그건 *기계적 적용* 의 위반. 둘 다 같이 살아 있으면
 * 도메인 무결성과 workflow 정합성이 *이중 방어선* 으로 보호된다.
 */
class IllegalJobLifecycleTransitionException : RuntimeException {
    constructor(source: JobStatus, event: JobLifecycleEvent) :
        super("허용되지 않은 transition: source=$source event=$event")

    constructor(source: JobStatus, event: JobLifecycleEvent, reason: String) :
