...
- 컨트롤러 → [`JobAccessControl#getOwned`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/JobAccessControl.kt)
...
- `GET /api/v1/jobs` (list) 는 `effectiveOwner = caller.isAdmin() && owner != null ? owner
  ...
  넘겨도 무시.
- 외부에 노출되는 에러 메시지는 [`GlobalExceptionHandler`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/exception/GlobalExceptionHandler.kt)
...
- 운영 ([`SecurityConfig`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/config/SecurityConfig.kt))
...
- 로컬 dev 는 [`PermissiveSecurityConfig`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/config/PermissiveSecurityConfig.kt)
...
- [`Caller.from`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/Caller.kt)
...
- `GET /api/v1/cost/summary` / `GET /api/v1/cost/top-spenders` 는 admin 전용. 비-admin
  ...
  체크를 명시적으로 둠 ([`CostController`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/CostController.kt)).
- 로그 출력 시 owner 는 [`OwnerLogMask`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/OwnerLogMask.kt)
  로 해시 마스킹, 이미지 reference 는 [`ImageLogMask`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/ImageLogMask.kt)
...
- [`CostQueryService.MAX_RANGE`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/CostQueryService.kt)
  = 366일. 더 큰 구간 요청은 400 — 호출자가 `Instant.EPOCH` 를 default 로 보내는
  사고 차단.
- `CostQueryService.MAX_TOP_N` = 100 — `?limit=` 무한대 차단.
- [`PreemptionController.MAX_RECENT_LIMIT`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/PreemptionController.kt)
  = 200 — admin 의 `?limit=` 도 응답 크기 폭증 방지.
- `JobSubmissionRequest.image` 는 정규식으로 256자 + 형식 제한, `inputUri` 는 `s3://`
  scheme 강제.
- [`QuotaService`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/QuotaService.kt)
...
- `JobSubmissionRequest.parentJobIds` 에 `@Size(max = 16)` 추가. 예전엔 size 캡 없음 →
  악성 / 실수 클라이언트가 수천 개 parent UUID 를 보내면 [`JobSubmissionService#validateNoCycle`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/JobSubmissionService.kt)
  의 BFS frontier 와 `findByChildJobIdIn` IN-쿼리 size 가 그에 비례해 폭주.
  ([`JobSubmissionRequest.kt`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/dto/JobSubmissionRequest.kt))
...
- admin 식별: `Caller.isAdmin()` — Spring Security `Authentication.getAuthorities()` 에서
  ...
  `JwtAuthenticationConverter` 가 자동 매핑 ([`SecurityConfig#jwtAuthConverter`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/config/SecurityConfig.kt)).
...
- `CALLBACK_SECRET` — [`KubernetesJobDispatcher#buildEnv`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/adapter/kubernetes/KubernetesJobDispatcher.kt)
...
- `X-GWP-Callback-Secret` 검증은 [`InternalCallbackController`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/InternalCallbackController.kt)
...
- 3차 ingress: nginx server-snippet 으로 `^/internal/` 은 404 — 외부 인터넷에서 워커
  콜백 path 가 보이지 않음.
- payload 검증: [`StatusCallbackRequest`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/dto/StatusCallbackRequest.kt)
  는 `@NotNull JobStatus status`, `@Size(max=1024) String resultUri`,
  `@Size(max=2048) String errorMessage` 로 길이 / 타입 한계 명시.
- lifecycle: [`JobLifecycleService#updateStatusFromCallback`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/application/JobLifecycleService.kt)
...
- `StatusCallbackRequest.resultUri` 에 `^$|^s3://[a-z0-9.\-]+/.+` 패턴 추가. 예전엔
  ...
  trust boundary 양쪽 ([`JobSubmissionRequest.inputUri`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/dto/JobSubmissionRequest.kt)
...
| 항목 | 위치 | 변경 |
| --- | --- | --- |
| API4 | [`JobSubmissionRequest.kt`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/dto/JobSubmissionRequest.kt) | `parentJobIds` 에 `@Size(max=16)` |
| API4 | [`application.yml`](../../orchestrator-api/src/main/resources/application.yml) | `spring.data.web.pageable.max-page-size: 100` |
| API10 | [`StatusCallbackRequest.kt`](../../orchestrator-api/src/main/kotlin/com/example/gwp/orchestrator/api/dto/StatusCallbackRequest.kt) | `resultUri` 에 `s3://` 패턴 강제 |
