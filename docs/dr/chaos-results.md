...
**상황**: 200ms 지연 주입 직후 p95 가 380ms 까지 올라감 (SLO 300ms 초과).

**원인**: HikariCP `idle-timeout` 이 너무 길어 idle connection 이 stale 한 채로 유지됨.

**조치**: `idle-timeout: 600s → 60s` 로 변경. 재실험에서 SLO 안에 들어옴.

### 05 gpu-node-drain (2026-06-12)

**상황**: 노드 강제 drain 시 워커 Pod 가 graceful shutdown 안 되고 즉시 죽음. orchestrator
는 SUCCEEDED/FAILED 콜백을 받지 못해 Job 이 RUNNING 상태로 30분간 유지됨.

**원인**: timeout 감시 작업이 미구현 (ADR-0003 의 follow-up 항목).

**조치**: Job timeout 감시 작업 (Spring Scheduling + ShedLock) 추가 예정. `RUNNING` 이
(timeout × 1.5) 넘으면 K8s 상태 자동 동기화.
