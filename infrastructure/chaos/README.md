...
Chaos engineering = 의도적으로 장애를 주입해서 시스템이 정말 회복되는지 검증.
Chaos Mesh (K8s 용 chaos 도구, CNCF graduated — Cloud Native Computing Foundation 의
graduated 등급 프로젝트) 로 운영 환경의 복원력을 정기적으로 검증합니다. 각 실험은
...
| 파일 | 가설 | 빈도 |
|---|---|---|
| `01-orchestrator-pod-kill.yaml` | PDB (PodDisruptionBudget — 동시 종료 가능 Pod 제한, minAvailable=1) 가 동작하여 다운타임 0 | 주 1회 |
...
| `05-gpu-node-drain.yaml` | GPU 노드 강제 drain (노드 강제 비우기) 시 Job 자동 재제출 | 분기 1회 |
...
- **자동 abort 기준** — error rate > 5% 또는 p99 latency (100건 중 99번째로 느린 응답) > 5s 이면 즉시 중단
...
Game Day = 정기적으로 일부러 장애를 만들고 운영 대응 절차를 훈련하는 행사. 분기별로
모든 실험을 한 번에 실행하는 game day 를 진행. 결과를
