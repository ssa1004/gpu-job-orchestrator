...
K6 = 스크립트 (JS) 로 시나리오를 짜는 부하 테스트 도구. orchestrator-api 의 Job 제출 /
콜백 / 조회 endpoint 에 대한 부하 테스트입니다.
...
이 목표를 어기면 K6 의 `thresholds` (시나리오에 적힌 통과 기준) 가 fail 처리하고 종료
코드를 nonzero (0 이 아닌 값 — CI 가 빌드를 실패로 판정) 로 반환합니다. CI 의 nightly
job (매일 야간 자동 실행) 에서 사용.
...
| callback-flood | 25ms | 180ms | 600ms | < 0.1% | OptimisticLock (낙관적 락 — 충돌 시 한쪽만 살리는 동시성 제어) retry 포함 |
